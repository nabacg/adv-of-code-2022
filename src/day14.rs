...
impl RockPath {
    ...
    fn new(p: Vec<(u32, u32)>) -> RockPath {
        ...
        let path_lines = p.iter().zip(p.iter().skip(1)).into_iter()
            .map(|(&s, &e)| (s, e))
            .collect::<Vec<((u32, u32), (u32, u32))>>();
        RockPath {  path_lines, lowest_level, left_most, right_most}
    }
}
...
impl fmt::Display for Cave {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        writeln!(f, "{}", (self.left_most..self.right_most+1).into_iter().map(|c| if c == self.left_most || c == self.right_most { c.to_string() } else { " ".to_string() }).join("")).expect("failed to write to fmt:Formatter");
        for y in 0..self.bottom_level+1 {
               let line = (self.left_most..self.right_most).into_iter().map(|x| {
                    match x {
...
impl Cave {
    ...
    fn is_free(&self, y: (u32, u32)) -> bool {
       !self.sand_units.contains(&y) 
        && self.rock_paths.iter().all(|p| !p.is_blocked(y))
    }
...
fn parse_point(p:&str) -> Result<(u32, u32), String> {
    ...
    .into_iter()
    .collect_tuple().ok_or(format!("failed to parse input line: {}", p))
}
