use core::fmt;
use std::{num::ParseIntError, ops::Index, error::Error};
...
impl TreeGrid {
    fn new(ls: String) -> Result<TreeGrid, String> {
        ...
        let data: Result<Vec<u32>, String> = 

        ls.chars().filter(|&c| c != '\n').map(|c| c.to_digit(RADIX).ok_or(format!("failed to parse char: {}", c))).collect::<Result<Vec<u32>, String>>();

        Ok(TreeGrid { tree_heights: Vec2d::new(data?, rows, cols) })
    }

    fn inner_trees(&self) -> Vec<TreeCoord> {
        (1..self.tree_heights.col-1).into_iter()
        .flat_map(|col| (1..self.tree_heights.row-1).map(|r| TreeCoord{ x: r, y: col}).collect::<Vec<TreeCoord>>())
        .collect()
    }
...
impl TreeCoord {
    fn is_visible(&self, tg: &TreeGrid) -> bool {
        ...
        !vec![left, up, right, down].iter().all(|p| {
            let covered =  p.path.iter().map(|c|tg.height(c)).any(|h| h >= this_height);
    ...
    fn scenic_score(&self, tg: &TreeGrid) -> usize {
        ...
        .iter()
        .map(|p| {
            let scenic_path = p.path.iter().take_while(|&c| { 
            ...
            println!("Tree: {} has scenic count of: {} from {}", self, scenic_count, p.dir);
            // if scenic_count == 0 {
            //     1
            // } else {
            //     scenic_count
...
impl VizPath {
    fn down(c: &TreeCoord, grid_rows: usize) -> VizPath {
        VizPath {
            dir: "DOWN".to_string(),
            path:
            (c.x+1..grid_rows).map(|r| TreeCoord {x: r, y: c.y}).collect()
        }   
    } 
    ...
    fn up(c: &TreeCoord) -> VizPath {
        VizPath {
            dir: "UP".to_string(),
            path:
            (0..c.x).rev() // it's easier if path Coords always go from TreeCord to the edge
            .map(|r| TreeCoord {x: r, y: c.y}).collect()
        }   
    } 
    ...
    fn right(c: &TreeCoord, grid_cols: usize) -> VizPath {
        VizPath {
            dir: "RIGHT".to_string(),
            path:
            (c.y+1..grid_cols).map(|col| TreeCoord {x: c.x, y: col}).collect()
        }   
    } 
    fn left(c: &TreeCoord) -> VizPath {
        VizPath {
            ...
            .map(|col| TreeCoord {x: c.x, y: col}).collect()
        }   
    } 
}
